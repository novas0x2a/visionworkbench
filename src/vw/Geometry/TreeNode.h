...
namespace geometry
{
  ...
  protected:
    ...
    //! Single-linked list of siblings.
    /**
  ...
  private:
    // Per class data of the @ancestry() method.
    //
  ...
  public:
    ...
    //! Copy constructor, to preserving the original tree.
    /**
    ...
    //! Assignement operator, preserving the original tree.
    /**
     * In order not to mess with the tree structure, only the data payload is copied.
     * So this creates a new root node, with no children.
     */
    ...
    DATA const& data() const throw() { return m_data; }
    //! Accessor method to the data.
    DATA & data() throw() { return m_data; }
     ...
     */
    TreeNode * parent() const throw() { return m_parent; }

    //! Predicate: returns true if node is root-node
    bool is_root() const throw() { return m_parent == NULL; };

    //! Predicate: returns true if node is a leaf-node
    bool is_leaf() const throw() { return m_first_child == NULL; };
